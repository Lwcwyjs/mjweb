...
var easyuiErrorFunction = function(XMLHttpRequest) {
    if(XMLHttpRequest.responseText.indexOf("登录")){
    	var str=parent.location.href;
    	parent.location.href=str.substr(0,str.indexOf("/admin"));
    }else{
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    strongPwd : {
        validator : function(value) {
            return getPasswordStrong(value);
        },
        message : '密码强度不够！'
    }
...
function resizeImg(obj,height){
	var he = 400;
	if(height>0){
		he = height;
	}
	var _this = $(obj);
	var HWratio_DIV =  _this.parents().width()/_this.parents().height(); //展示区域宽高比
	var PERratio; //原始图片宽高比
	
	var FinalWidth;
	
	var url = _this.attr("src"); //原始图片地址值
	var perImg = new Image();//声明原始图片
	perImg.src = url;	//
	console.log("原始照片url！"+url);

	if(perImg.complete){

		PERratio = perImg.width/perImg.height;
		FinalWidth = he/perImg.height*perImg.width;
		
		console.log("比率："+PERratio);
		//固定高度
		//FinalWidth = 400 * PERratio;
		console.log("最终展示区域宽度！"+FinalWidth);
		if(FinalWidth>=_this.parents().width()){
			console.log("最终宽度大于展示区域！");
			_this.width(_this.parents().width());
			_this.height("'"+he+"'");
			
		}else{
			_this.width(FinalWidth);
			_this.height("'"+he+"'");
		}
		...
		perImg.onload = function(){
			...
			PERratio = Math.floor(perImg.width/perImg.height * 100) / 100;
			FinalWidth = he/perImg.height*perImg.width;
			console.log("比率："+PERratio);
			//固定高度
			console.log("最终展示区域宽度！"+FinalWidth);
			if(FinalWidth>=_this.parents().width()){
				...
				_this.animate({width:_this.parents().width()+'px', height:he+'px',});
				
			}else{

				_this.animate({width:FinalWidth+'px', height:he+'px',});
			}
