...
<div class="layui-fluid">
			...
			<div class="layui-form toolbar" id="dataBaseSearchForm">
				<div class="layui-form-item">
					...
					<div class="layui-inline">
						...
						<div class="layui-input-inline">
							<input name="jkid" id="jkid" class="layui-input"/>
						</div>
					</div>
	...
	<script type="text/html" id="actionHandle">
		<shiro:hasPermission name="/admin/invokinglog/edit">
...
<script>
	layui.config({
		...
		table.render({
			elem: '#vehicleTable'
			, url: '${path}/admin/invokinglog/dataGrid'
			, method: 'post'
			, request: {
			...
			, cols: [[
				{field: 'user_name', title: '请求方名称', align: 'center', sort: true
				}, {
					field: 'jkid', title: '接口id',align: 'center', sort: true
				}
				, {
					field: 'terminal_id', title: '请求IP',align: 'center', sort: true
				}
				, {
					field: 'operate_time', title: '接口服务时间', align: 'center', sort: true,
					templet: function (d) {
				...
				, {
					field: 'operate_content', title: '接口服务条件',align: 'center'
				}
				, {
					field: 'operate_result', title: '操作返回', align: 'center', sort: true
				}
				, {
		...
		table.on('tool(vehicleFilter)', function (obj) {
			...
			if (obj.event === 'detail') {
				...
				parent.layer.open({
					...
					area: ['800px', '500px'],
					skin: 'my-skin',
					content: ['${path}/admin/invokinglog/editPage?id=' + data.num_id]
				});
			}
		});
