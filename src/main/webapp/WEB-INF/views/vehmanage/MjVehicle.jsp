...
<script>
    layui.config({
        ...
        var getCurrentPage = function () {
            var $view = $('#vehTable').next('.layui-table-view');
            if ($view.length === 0) {
                $view = $('#vehTable').next();
            }
            var text = $view.find('.layui-laypage-em').next().text();
            var curr = parseInt(text, 10);
            return isNaN(curr) ? 1 : curr;
        };
        ...
        $(document).on('click', '#addDetail', function () {
            ...
            parent.layer.open({
                ...
                skin: 'my-skin',
                area: ['800px', '600px'],
                content: ['${path}/business/mjvehicle/addPage'],
                end: function () {
        ...
        table.on('tool(vehicleFilter)', function (obj) {
            ...
            if (obj.event === 'editDetail') {
                ...
                parent.layer.open({
                    ...
                    area: ['800px', '700px'],
                    skin: 'my-skin',
                    content: ['${path}/business/mjvehicle/editPage?id=' + data.id],
                    end: function () {
                        table.reload('vehTable', {
                            page: {curr: currPage},
                            where: getWhereParams()
