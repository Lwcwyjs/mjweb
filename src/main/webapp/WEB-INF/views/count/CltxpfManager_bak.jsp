...
<%--                    </div>--%>
    ...
    <div class="layui-col-xs6 layui-col-sm12 layui-col-md12" style="display: flex;justify-content: center;">
        <div id="cltxpfPie" style="width: 50%; height:42vh; margin: 10px;"></div>
    </div>
...
<script>
    ...
    layui.use(['element','form', 'table', 'tree', 'laydate', 'formSelects'], function () {
        ...
        $('#exportExcel').click(function () {
            ...
            var jssj = $('#jssj').val();
            var cpys = $('#cpys').val();
            window.location.href='${path}/count/cltxpf/getExcel?jclx='+jclx+'&cpys='+cpys+'&organ='+organ+'&kssj='+kssj+'&jssj='+jssj;
        });
        laydate.render({
            elem: '#kssj'
        });
            ...
            series: [
                {
                    ...
                    data: [
                        {
                            value: g4Num, name: '国V以下', itemStyle: {
                                normal: {
                        ...
                        {
                            value: g5Num, name: '国Ⅴ', itemStyle: {
                                normal: {
                                    color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                                        offset: 0,
                                        color: 'rgba(107,147,35)'
                                    }, {
                                        offset: 1,
                                        color: 'rgba(107,147,35)'
                                    }]),
                        ...
                        {
                            value: xNum, name: '未录入', itemStyle: {
                                normal: {
                                    color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                                        offset: 0,
                                        color: 'rgba(255,153,18)'
                                    }, {
                                        offset: 1,
                                        color: 'rgba(255,153,18)'
                                    }]),
    ...
    function getChartData(params) {
        $.ajax({
            type: "post",
            url: '${path}/count/cltxpf/cltxpfsj',
            data: params,
            dataType: 'json',
            success: function (cltxpfList) {
                if(cltxpfList!=""){
                    ...
                    $("#cltxptable").html("");
                    shtml="<table>"
                    shtml = shtml + '<tr><td>排放阶段</td><td>车辆数</td><td>进出辆次</td></tr>';
                    for (var i = 0; i < cltxpfList.length; i++) {
                        shtml = shtml + '<tr><td>'+cltxpfList[i].pfbz+'</td><td>'+cltxpfList[i].cls+'</td><td>'+cltxpfList[i].jclc+'</td></tr>';
                    }
                    shtml=shtml+"</table>";
        ...
        $.ajax({
            type: "post",
            url: '${path}/count/cltxpf/cltxpfpie',
            data: params,
            dataType: 'json',
            success: function (d) {
                // $("[selectOrgan]").html("");
