...
$(document).ready(function() {
	/*
	 * var w=art.dialog.data('width')*1-325; var
	 * h=art.dialog.data('height')*1-45;
	 * $("#Vehicle").html(art.dialog.data('vehicle'));
	 * ShowImg(art.dialog.data('src'),w,h);
	 * 
	 * art.dialog.removeData('width'); art.dialog.removeData('height');
	 * art.dialog.removeData('src'); art.dialog.removeData('vehicle');
	 */
});
function ShowImg(div, src, w, h) {
	if (w == null || w == "" || w == 0) {
		w = $(document).width() * 1 - 225;
	}
	if (h == null || h == "" || h == 0) {
		h = $(document).height() * 1 - 95;
	}
	$("#" + div).empty();
	$("#" + div).width(w);
	$("#" + div).height(h);
...
/*
 * ! ImageTrans Copyright (c) 2010 cloudgamer Blog:
 * http://cloudgamer.cnblogs.com/ Date: 2010-8-15
 */
...
ImageTrans.prototype = {
	...
	load : function(src) {
		if (this._support) {
			...
			//自适应显示
			/*var hei = this._clientHeight > img.offsetHeight ? img.offsetHeight
					: this._clientHeight;
			var wid = this._clientWidth > Math.floor(img.offsetWidth * hei
					/ img.offsetHeight) ? Math.floor(img.offsetWidth
					* hei / img.offsetHeight) : this._clientWidth;
			hei = this._clientWidth > wid ? hei : Math
					.floor(img.offsetHeight * wid / img.offsetWidth);
	...
	return {
		css3 : {// css3设置
			support : function() {
				...
				return $$A.some([ "transform", "MozTransform",
						"webkitTransform", "OTransform" ], function(css) {
					if (css in style) {
						css3Transform = css;
						return true;
					}
		...
		filter : {// 滤镜设置
			...
			show : function() {
				...
				// 保持居中
				img.style.top = ((this._clientHeight - img.offsetHeight) / 2 > 0 ? (this._clientHeight - img.offsetHeight) / 2
						: 0)
						+ "px";
				img.style.left = ((this._clientWidth - img.offsetWidth) / 2 > 0 ? (this._clientWidth - img.offsetWidth) / 2
						: 0)
						+ "px";
			},
			dispose : function() {
		...
		canvas : {// canvas设置
			...
			init : function() {
				var canvas = this._canvas = document.createElement('canvas'), context = this._context = canvas
						.getContext('2d');
				// 样式设置
				$$D.setStyle(canvas, {
					position : "absolute",
...
ImageTrans.transforms = {
	...
	scale : function() {
		...
		return function(zoom) {
			if (zoom) {
				var hZoom = getZoom(this._y, zoom), vZoom = getZoom(this._x,
						zoom);
				if (hZoom && vZoom) {
					this._y += hZoom;
					this._x += vZoom;
...
ImageTrans.prototype._initialize = (function() {
	...
	// 拖动函数
	function move(e) {
		this.rotate(Math.atan2(e.clientY - this._mrY, e.clientX - this._mrX)
				- this._mrRadian);
...
ImageTrans.prototype._initialize = (function() {
	...
	// 缩放函数
	function zoom(e) {
		this.scale((e.wheelDelta ? e.wheelDelta / (-120) : (e.detail || 0) / 3)
				* Math.abs(this._zoom));
		e.preventDefault();
	}
	;
