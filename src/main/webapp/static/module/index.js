...
layui.define(["layer", "element", "admin", "contextMenu"], function(s) {
	...
	var h = {
			...
			if (h.pageTabs) {
				...
				if (!v) {
					...
					b.tabAdd(i, {
						id: x,
						title: w ? w : "无标题",
						content: '<iframe lay-id="' + x + '" src="' + x + '" allowfullscreen="true" frameborder="0" class="admin-iframe"></iframe>'
					});
					if (x != c) {
						h.mTabList.push(z)
					}
				...
				if (!u || u.length <= 0) {
					...
					// y += "      </span>";
					// y += "   </div>";
					var y = '   <div style="-webkit-overflow-scrolling: touch;">';
		...
		loadMenu: function() {
			d('.layui-layout-admin .layui-side .layui-nav a[lay-href]').each(function () {
				var menuPath = d(this).attr('lay-href');
				if ('javascript:;' != menuPath && '' != menuPath) {
					d(this).attr('lay-href', basePath + menuPath);
				} else {
					d(this).attr('lay-href', 'javascript:;');
				}
			});
		},
		loadHome: function(w) {
