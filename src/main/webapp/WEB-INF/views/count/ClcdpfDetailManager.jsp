...
<script>
    ...
    layui.use(['element','form', 'table', 'tree', 'laydate', 'formSelects'], function () {
        ...
        $('#exportExcel').click(function () {
            ...
            var jssj = $('#jssj').val();
            var cpys = $('#cpys').val();
            window.location.href='${path}/count/clcdpfDetail/getExcel?jclx='+jclx+'&cpys='+cpys+'&organ='+organ+'&kssj='+kssj+'&jssj='+jssj;
        });
        laydate.render({
            elem: '#kssj'
        });
    ...
    function getChartData(params) {
        $.ajax({
            type: "post",
            url: '${path}/count/clcdpfDetail/clqjyswsj',
            data: params,
            dataType: 'json',
                ...
                if(cltxpfList!=""){
                    ...
                    for (var i = 0; i < cltxpfList.length; i++) {
                        ...
                        if(isfirst==1) {
                            ...
                            else if(currentyshwmc=="皮带传输比例（%）"){
                                shtml = shtml + '<tr><td colspan="2">' + cltxpfList[i].yshwmc + '</td><td colspan="3">' + cltxpfList[i].yslzb + '</td></tr>';
                            }
                            else if(currentyshwmc=="超低排放比例（%）"){
                                shtml = shtml + '<tr><td colspan="2">' + cltxpfList[i].yshwmc + '</td><td colspan="3">' + cltxpfList[i].yslzb + '</td></tr>';
                            }
                            else {
