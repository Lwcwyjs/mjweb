...
var SimpleCalendar = function () {
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function updateSelect(year, month) {
            ...
            for (var i = this._options.timeRange.startMonth; i < this._options.timeRange.endMonth; i++) {
        ...
        value: function addData(year, month) {
            ...
            for (var i = 0; i < 35; i++) {
                ...
                if (this.tyear == writeyear && this.tday == writeday && this.tmonth == writemonth) {
                    ...
                    writemonth = writemonth < 10 ? "0" + writemonth : writemonth;
                    writeday = writeday < 10 ? "0" + writeday : writeday;
                    localStorage.setItem("bookingDate", writeyear + "-" + writemonth + "-" + writeday);
                }
            }
        }
            ...
            if (options.showMark) {
                ...
                if (data) {
                    daysElement.forEach(function (v, i) {
                        ...
                        if (currentmonth == 0) {
                            var currentmonth1 = 12
                        } else {
                            var currentmonth1 = currentmonth < 10 ? "0" + currentmonth : currentmonth;
                        }
                        if (year + '-' + currentmonth1 + '-' + day == "2020-12-31") {
                           v.classList.add('sc-othermenth');
                                v.classList.remove('sc-mark');
                                v.title = '';
                        } else {
                            if (data[year + '-' + currentmonth1 + '-' + day]) {
                                v.classList.add('sc-mark');
                                v.title = data[year + '-' + currentmonth1 + '-' + day];
                                v.querySelector('.lunar-day').innerHTML = data[year + '-' + currentmonth1 + '-' + day];
                            } else {
                                v.classList.add('sc-othermenth');
                                v.classList.remove('sc-mark');
                                v.title = '';
            ...
            daysElement.forEach(function (v, i) {
                v.onclick = function () {
                    if (!v.classList.contains("sc-othermenth")) {
                        ...
                        if (pre) pre.classList.remove('sc-today');
                        this.classList.add('sc-selected');
                        this.classList.add('sc-today');
                        var selectYear = container.querySelector('.sc-select-year');
                        var selectMonth = container.querySelector('.sc-select-month');
                        var selectDay = v.querySelector('.day').innerHTML;
                        selectDay = selectDay < 10 ? "0" + selectDay : selectDay;
                        var month = selectMonth.value < 10 ? "0" + selectMonth.value : selectMonth.value;
                        localStorage.setItem("bookingDate", selectYear.value + "-" + month + "-" + selectDay);
                    }
                };
            });
